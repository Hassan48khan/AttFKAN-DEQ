# model/deq_core.py
# Full Deep Equilibrium (DEQ) Transformer Core Implementation
# This file contains the complete DEQ-Transformer architecture as provided,
# including weight-shared attention, position-wise feed-forward, relative positional encoding,
# Anderson/Broyden solvers, Jacobian loss, spectral radius estimation, and memory handling.
# This core is used as the foundation for integrating with AttFKAN blocks in AttFKAN-DEQ.
...
# Append necessary paths (adjust based on your project structure)
sys.path.append('../../')
...
class WeightSharePositionwiseFF(nn.Module):
    """
    Weight-shared Position-wise Feed-Forward Network with variational dropout.
    """
...
class WeightShareSelfAttention(nn.Module):
    """
    Weight-shared Multi-Head Self-Attention with Relative Positional Encoding (similar to Transformer-XL).
    """
    ...
    def forward(self, z1ss, pos_emb, u1ss, mems=None):
        ...
        w_head_q = w_head_q.view(bsz, n_head, d_head, qlen)
        w_head_k = w_head_k.view(bsz, n_head, d_head, klen)
        w_head_v = w_head_v.view(bsz, n_head, d_head, klen)
        r_head_k = r_head_k.view(n_head, d_head, rlen)
...
class RelPartialLearnableDecoderLayer(nn.Module):
    """
    Single decoder layer combining self-attention and position-wise feed-forward.
    """
...
class DEQTransformerLM(nn.Module):
    """
    Deep Equilibrium Transformer Language Model.
    This is the full DEQ core with support for Anderson/Broyden solvers, Jacobian regularization,
    spectral radius estimation, and memory caching.
    """
    ...
    def _update_mems(self, z1s, us, z0, qlen, mlen):
        ...
        with torch.no_grad():
            ...
            new_z0 = zs[:, :, beg_idx:end_idx].detach().permute(2, 0, 1).contiguous()
    ...
    def _forward(self, dec_inp, mems=None, f_thres=30, b_thres=40, train_step=-1,
        ...
        u1s = self.inject_conv(word_emb.transpose(1, 2))
        z0, u0 = mems
        ...
        pos_emb = self.pos_drop(self.pos_emb(pos_seq))
        us = torch.cat([u0, u1s], dim=2)
        z1s = torch.zeros(bsz, d_model, qlen)
        ...
        core_out = self.iodrop(new_z1s, self.dropout).permute(2, 0, 1).contiguous()
    ...
    def forward(self, data, target, mems, train_step=-1, **kwargs):
        if not mems:
            ...
            for i in range(len(mems)):
                mems[i] = mems[i].permute(1, 2, 0).contiguous()
        qlen, bsz = data.size()
